...
We manually created GRL models from argument schemes and critical questions found in each transcript, to verify whether the arguments put forward by the participants were sufficiently informative. An example of such a model is shown in Figure~\ref{fig:transcripts:grl}. 
%TODO Again the text and the figure don't match.
%MvZ: What doesn't match? We only say here we give an example in Figure 6, so there is not problem, right?
...
%MvZ: The examples below were not meant to correspond to anything, I just wrote them as ficitious examples. Looking at your remarks I think this is confusing.
...
%TODO In figure it is XOR %MvZ: Fixed
Suppose now the critical question CQ5b: ``Does the goal \texttt{Generate cars} decompose into other tasks?'' is answered with ``Yes, namely \texttt{Choose randomly}''. This results in a new instantiation of AS5, namely: ``Goal \texttt{Generate cars} XOR-decomposes into tasks \texttt{Keep same cars}, \texttt{Create new cars}, and \texttt{Choose randomly}.'' As a result, the goal model will contain the corresponding task \texttt{Choose randomly}, as well as an OR-decomposition relation from the goal \texttt{Generate cars} to that task. %TODO ?????
