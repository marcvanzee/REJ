...
We assume familiarity with basic notions from propositonal logic and set theory. A \emph{propositional atom} is a grounded formula (a formula without variables) with no logical connectives ($\vee, \and, \rightarrow, \leftrightarrow$) or negation ($\not$). We use integers $i,j\in\mathbb{N}$ as identifiers for actors and GRL elements.
