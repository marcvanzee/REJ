...
\item An labeling of the elements, assigning to each element (an IE, link, or generic argument) the label IN (accepted) or OUT (reject). In the next section we explain how the labeling is computed, but in this section we only provide a formal definition.
\end{itemize}

Furthermore, we also change the representation of IE, links, and actors. In GRL, these elements have a single identifier. However, in RationalGRL, we allow the user to \emph{replace} such elements when answering a critical question with effect REPLACE. Therefore, for each GRL element, we maintain a history, which is a list in which the last element of the list corresponds to the element that ends up in the GRL model.
...
We next add the the labeling of elements. This labeling determines whether an element is accepted (which means it shows up in the GRL model if it is an IE or a link), or rejected (which means it won't be shown).
...
Finally we add the last part of the RationalGRL extension. We define a list of intentional elements, contribution links, and actors.

\begin{definition}[IE History, Contribution History, Actor History]
\label{def:element-list}
\begin{itemize}
\item An \emph{IE History for element with id $i$}, denoted $\overline{ie}_i=(ie_0,\ldots,ie_n)$, is a list of IEs $ie_0,\ldots,ie_n$ such that $ie_1.id=\ldots=ie_n.id=i$. 
\item An \emph{Actor history for actor with id $j$}, denoted $\overline{act}_j=(actor_0,\ldots,actor_n)$, is a list of actors $actor_0,\ldots,actor_n$ such that $actor_0.id=\ldots=actor_n.id=j$. 
\item A \emph{Contribution history for link with id $k$}, denoted $\overline{contrib}_j=(contrib_0,\ldots,contrib_n)$, is a list of contribution links $contrib_0,\ldots,contrib_n$ such that $contrib_0.id=\ldots=contrib_n.id=k$.
\end{itemize}
If the id in the list is not relevant we may omit the subscript notation, e.g., we may write $\overline{ie}$ instead of $\overline{ie}_i$.

Sets of histories for IEs, actors, and contribution links are denoted with respectively $\overline{IE}$, $\overline{Act}$, and $\overline{Contrib}$. 

The last element of a list is called the \emph{head element}.
\end{definition}

Note that we do not require a history for decomposition links or dependency links. This is because these links do not have associated REPLACE operations.

\begin{definition}[RationalGRL Model]
\label{def:rationalgrl-model}
A \emph{RationalGRL model} $RationalGRL=(\overline{IE}, \overline{Act}, R_{ActIE}, \overline{Contrib}, Decomp, Dep, Arg, Att, Label)$ consists of:
\begin{itemize}
\item A set $\overline{IE}$ of lists of intentional elements (Def.~\ref{def:element-list}),
\item A set $\overline{Act}$ of lists of actors (Def.~\ref{def:element-list}),
\item A set $R_{ActIE}$ of Actor-IE relations (Def.~\ref{def:act-ie-relation}),
\item A set $\overline{Contr}$ of lists of contribution links (Def.~\ref{def:element-list}),
\item A set $Decomp$ of decomposition links (Def.~\ref{def:link:decomp}),
\item A set $Dep$ of dependency links (Def.~\ref{def:link:dependency}).
...
\begin{enumerate}
\item ids are globally unique across IEs, Links, and Actors, i.e., let $X,Y\in \{\overline{IE},\overline{Act}, \overline{Contr}, Decomp, Dep\}$. For all $X_i$ and $Y_j$: if $i=j$ then $X=Y$ and $X_i=Y_j$.
\item All intentional elements of actors exist: $\forall (i,j)\in R_{ActIE}: \overline{act}_i \in \overline{Act} \wedge ie_j \in IE$.
\item An intentional element belongs at most to one actor: $\forall ie_i\in IE: |\{(k,i)\in R_{ActIE}\}| \le 1$.
\item Contribution links connect intentional elements: $\forall (i,j,k,value)\in \overline{Contrib}: \{ie_j,ie_k\}\subseteq IE$.
\item Decomposition links connect intentional elements: $\forall (i,j,k)\in Decomp: \{ie_j,ie_k\}\subseteq IE$.
\item Dependency links connect intentional elements: $\forall (i,j,)\in Dep: \{ie_j,ie_k\}\subseteq IE$.
...
\begin{algorithm}[h]
  \caption{Translation Algorithm}\label{alg:translation}
  \begin{algorithmic}[1]
    \State TODO
  \end{algorithmic}
\end{algorithm}
...
\begin{theorem}
If a RationalGRL model $m$ satisfies the conditions of a RationalGRL model (Def.~\ref{def:rationalgrl-model}), then $Trans(m)$ satisfies the conditions of a GRL model (Def.~\ref{def:grl-model}).
\end{theorem}
