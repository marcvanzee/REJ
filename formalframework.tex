...
In Fig.~\ref{fig:example-small3}, the actor, all the intentional elements, all links with identifiers, and the generic counter argument are arguments. We will give a full formalization of this after the definition of a RationalGRL model (Def.~\ref{def:rationalgrl-model}.
...
\paragraph{Rational and example} The definition of a GRL model collects all the previously defined tuples into a single definition. For completeness, we now provide the full specification of Figure~\ref{fig:example-small3}. This model is formalized as
 $RationalGRL=(Arg, Att)$ where:
\begin{flalign*}
Arg=&\{((2, task, \text{Car objects}, AND), Task),&\\
   &  ((3, softgoal, \text{Real. sim.}, AND), Softgoal),&\\
   &  ((4, softgoal, \text{Simple des.}, AND), Softgoal),&\\
   &  ((5, goal, \text{Generate traffic}, XOR), Goal),&\\
   &  ((6, task, \text{Create new cars}, AND), Task),&\\
   &  ((7, task, \text{Keep same cars}, AND)\}, Task),&\\
   &  ((0, \text{Traffic Simulator}), Actor),&\\
   &  \{((0,i), ActIE)\mid 2\le i\le 7\},&\\
   &  ((9, 6, 3, \text{Help}), Contr),&\\
   &  ((10, 6, 4, \text{Hurt}), Contr),&\\
   &  ((11, 6, 5), Decomp),&\\
   &  ((12, 7, 5), Decomp),&\\
   &  ((8, 5, 2), Dep),&\\
   &  ((13, \text{Redundant}), GenArg),&\\
Att=&\{(13,2)\}
\end{flalign*}
...
\centering
\begin{tikzpicture}
        \node[minimum size=1cm] (b) [argNodeIN] at (-3,0) {Car objects ($A_1$)};
        \node[minimum size=1cm] (c) [argNodeIN] at (0,0) {Redundant ($A_2$)};
        \node[minimum size=1cm] (d) [argNodeIN] at (3,0) {Necessary ($A_3$)};
