...
%SG: We need to write a bit about the framework in past and refer it here. 
To answer the research question above and solve some of the concerns mentioned in Subsection~\ref{sect:problem_statement}, we propose a framework called RationalGRL~\cite{}. The RationalGRL framework combines the Goal-oriented Requirements Language (GRL) with a technique from argumentation and discourse modeling called \emph{argument schemes}~\cite{walton-etal2004}. %SG: we need to add 1-2 sentences about our previous work and then add what we do here... 
We develop a metamodel specifying how our framework extends GRL, and we use this as the basis for a prototype web-based implementation~\footnote{The tool is available at <GITHUB LINK>}. %SG: Placeholder for the link. 
